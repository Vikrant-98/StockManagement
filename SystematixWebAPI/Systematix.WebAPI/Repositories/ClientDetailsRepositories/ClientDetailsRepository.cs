...
namespace Systematix.WebAPI.Repositories.ClientDetailsRepositories
{
    public class ClientDetailsRepository : IClientDetailsRepository
    {
        ...
        public async Task<Response> LoginClientAsync(ClientInformationRequest employeedetail)
        {
            ...
            var ClientInformation = await systematixDbContext.tbl_Client.FirstOrDefaultAsync(x => x.EmailID == employeedetail.EmailID 
        ...
        public async Task<(bool, string)> RegisterClientAsync(ClientInformation ClientRegister)
        {
            ...
            return (true, "User Register Succesfully");
        }
        ...
        private async Task<ClientHoldingResponse> ClientResponse(string EmailId) 
        {
            ...
            foreach (var item in ClientHoldingGroup)
            {
                ...
                var TempSymbol = item.Select(x => x.ClientCode).FirstOrDefault();
                var TempISIN = item.Select(x => x.ClientCode).FirstOrDefault();
